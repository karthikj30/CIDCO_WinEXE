...
public sealed class PortalSender : ICidcoTransport
{
    ...
    public SendResult Send(FileInfo? file = null)
    {
        ...
        {
            return SendResult.Failed(Diagnose(error), TransferOutcome.Unreachable)
                with { FileName = remoteName, Remote = UploadUri.ToString() };
        }

        using (response)
        {
            var sent = new SendResult(true, $"{source.Name} renamed to {remoteName} and sent to CIDCO")
            {
                FileName = remoteName,
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                return detail.Length > 0
                    ? sent with { Message = $"{source.Name} renamed to {remoteName} and sent to CIDCO — {detail}" }
                    : sent;
            }
            ...
            var message = outcome == TransferOutcome.BadCredentials
                ...
                : $"{remoteName} — CIDCO refused the transfer" + (said.Length > 0 ? $" ({said})" : $" ({(int)response.StatusCode})");
